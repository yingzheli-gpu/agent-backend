"""MiniMax 聊天模型"""
from typing import Any, Iterator, AsyncIterator, List, Optional, Tuple
...
class Chat(BaseChatOpenAI):
    ...
    支持模型: abab6.5-chat, abab5.5-chat 等
    API兼容OpenAI格式
    
    支持 Thinking Mode (需要设置 enable_thinking=True)
    """
    
    model: str = Field(default="abab6.5-chat")
    ...
    def _build_request_params(self, openai_messages: List[dict]) -> dict:
        ...
        request_params = {
            ...
            "extra_body": {"reasoning_split": True},  # MiniMax 使用 reasoning_split
        }
    ...
    def _convert_response_to_langchain_format(self, response) -> Iterator[ChatGenerationChunk]:
        ...
        for chunk in response:
            ...
            # MiniMax 特殊处理：提取 reasoning_details
            # 使用 getattr 因为 reasoning_details 可能不在 __dict__ 中
            reasoning_details = getattr(delta, 'reasoning_details', None)
            if reasoning_details:
                # reasoning_details 是列表，每个元素是字典，包含 'text' 字段
                if isinstance(reasoning_details, list) and len(reasoning_details) > 0:
                    reasoning_detail = reasoning_details[0]
                    if isinstance(reasoning_detail, dict) and 'text' in reasoning_detail:
                        additional_kwargs['reasoning_content'] = reasoning_detail['text']
