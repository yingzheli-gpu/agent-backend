...
logger = get_logger("intent_router.final")
...
class IntentRouter:
    ...
    async def route(
        ...
        Args:
            ...
            has_image: 是否包含图片
            enriched_context: 主图中间件已构建的增强上下文（用户画像、环境、对话历史）
                              只传递给 LLM 分类器，规则层不使用
        ...
        # 使用主图传入的上下文，无则降级为空上下文
        if isinstance(enriched_context, EnrichedContext):
        ...
        llm_query = intent_query or query
        if llm_query != query:
            route_path.append("L3:contextualized_query")
