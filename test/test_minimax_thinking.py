...
def test_minimax_sync_stream():
    ...
    print("=" * 80)
    print("测试 1: MiniMax 同步流式 (stream) - 启用 thinking mode")
    print("=" * 80)
...
async def test_minimax_async_stream():
    """测试 2: 异步流式 (astream) - 启用 thinking mode"""
    
    print("\n" + "=" * 80)
    print("测试 2: MiniMax 异步流式 (astream) - 启用 thinking mode")
    print("=" * 80)
...
def test_minimax_invoke():
    ...
    print("\n" + "=" * 80)
    print("测试 3: MiniMax 非流式 (invoke) - 启用 thinking mode")
    print("=" * 80)
...
async def test_minimax_ainvoke():
    """测试 4: 异步非流式 (ainvoke) - 启用 thinking mode"""
    
    print("\n" + "=" * 80)
    print("测试 4: MiniMax 异步非流式 (ainvoke) - 启用 thinking mode")
    print("=" * 80)
...
async def run_all_tests():
    ...
    print("\n⚠️ 请先在脚本中设置你的 MiniMax API Key！\n")
    
    if API_KEY == "your-minimax-api-key":
    ...
    # 测试 1: 同步流式
    results["同步流式 (stream)"] = test_minimax_sync_stream()
    
    # 测试 2: 异步流式
    results["异步流式 (astream)"] = await test_minimax_async_stream()
    
    # 测试 3: 非流式
    results["非流式 (invoke)"] = test_minimax_invoke()
    
    # 测试 4: 异步非流式
    results["异步非流式 (ainvoke)"] = await test_minimax_ainvoke()
    ...
    if all_passed:
        print("\n🎉 所有测试通过！MiniMax thinking mode 集成成功！")
    else:
        print("\n⚠️ 部分测试失败，请检查错误信息")
